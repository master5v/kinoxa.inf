...

{
    ...
    if ($_GET[mac]) {
        $box_mac = $_GET[mac];
        $box_mac = trim($box_mac);
        $box_mac = mysql_real_escape_string($box_mac);
        $box_mac = htmlspecialchars($box_mac);
        $_SESSION[user][mac] = $box_mac;
    }
    $box_mac = $_SESSION[user][mac];
    $userMac = userMac($box_mac);
    $userMac = $userMac['0'];
    ...
    if ($userMac) {
        ...
        if (isset($_GET[uid])) {
            ...
            if ($upduser) {
                echo "Вы авторизированы. Приятного пользования порталом!";
                unset($_SESSION[user][mac]);
            } else {
